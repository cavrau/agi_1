...
(async function () {
  try {
    client = await MongoClient.connect(BD_URL, { useNewUrlParser: true, useUnifiedTopology: true })
    db = client.db()
...
async function leUsuarios () {
  const col = db.collection(COL_USUARIOS)
  const usuarios = await col.find({}, {login: 1})
...
export {
  leUsuario, leUsuarios, cadastraUsuario,
